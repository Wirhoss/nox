			...
			<aside>
				<section class="configuration-panel">
					...
					{:else}
						...
						<div class="participant-list">{#each $detail.participantBlueprintIds as id}<span><i>{id.slice(0, 1).toUpperCase()}</i>{participantName(id)}</span>{/each}</div>
					{/if}
				</section>
			</aside>
		</div>
